...
async function addTask(data, teamid) {
    try {
...
async function getSimilarArticle(topic) {
    try {
        let articles = await pb.collection('ARTICLE').getFullList({
            sort: '-created',
            filter: `topic = "${topic}"`
...
async function getSomePost(currentpage, nbrOfPost) {
    try {
...
async function getRecentComment(userId) {
    try {
...
async function getTeamBoard(id) {
    try {
...
async function getPostCommentNB(postId) {
    let total = 0;
...
async function getRecursiveCommentNB(id) {
    let NB = 0;
...
async function getCommentTree(comments) {
...
export { getSimilarArticle as a, getAllArticle as b, addJam as c, getPostPage as d, getSomePost as e, getRecentComment as f, getArticle as g, addGame as h, getGame as i, getTeamBoard as j, addTask as k, getUserTeams as l, getPopularJam as m, getUser as n, getJamPage as o, getAllJamFiltered as p, getRecentArticle as q };
