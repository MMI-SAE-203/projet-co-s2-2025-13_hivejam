...
export async function addGame(data, jamid, teamid) {
    try {
        const game = await pb.collection('GAME').create(data);

        const game_jamRecorde = await pb.collection("GAME_JAM").getOne(jamid);
        await pb.collection("GAME_JAM").update(jamid, { games: [...game_jamRecorde.games, game.id] });
        await pb.collection("GAME").update(game.id, { team: teamid});
